...
from integrals.eri import compute_eri_laplace, compute_eri
...
def plot_eri_convergence_direct():
    ...
    levels = list(range(4, 12))  # Cubature levels to test
    eri_values = []
...
def plot_eri_convergence_laplace():
    ...
    for s_level in s_levels:
        ...
        eri_value = compute_eri_laplace(alpha, center1, center2, center3, center4, level=6, s_min=0.001, s_max=1000.0, s_level=s_level)
        end = perf_counter()
        elapsed = end-start
        print(s_level,eri_value,elapsed,flush=True)
